<template>
  <main class="lg:flex lg:min-h-full lg:flex-row-reverse lg:overflow-hidden max-w-screen-2xl mx-auto mt-32">
    ...
    <section aria-labelledby="order-heading" class="bg-gray-50 px-4 py-6 sm:px-6 lg:hidden">
      <div class="mx-auto max-w-lg">
        <div class="flex items-center justify-between">
          <h2 id="order-heading" class="lgLtext-lg font-medium text-gray-900">Your Order</h2>
          <button @click="showOrderSummary = !showOrderSummary" type="button"
            class="font-medium text-indigo-600 hover:text-indigo-500" aria-controls="disclosure-1"
            aria-expanded="false">
            <!-- Only display for open option. -->
            <span v-if="showOrderSummary">Hide full summary</span>
            <!-- Don't display for open option. -->
            <span v-if="!showOrderSummary">Show full summary</span>
          </button>
        </div>

        <div id="disclosure-1" v-if="showOrderSummary">
          <ul role="list" class="divide-y divide-gray-200 border-b border-gray-200">
            <li class="flex space-x-6 py-6" v-for="itm in 3" :key="itm">
          ...
          <form class="mt-10">
            ...
            <div class="mt-1 flex space-x-4">
              <input type="text" id="discount-code-mobile" name="discount-code-mobile"
                class="block w-full rounded-md border-gray-300 shadow-sm border focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
              <button type="submit"
        ...
        <form>
          ...
          <div class="mt-1 flex space-x-4">
            <input type="text" id="discount-code" name="discount-code"
              class="block w-full rounded-md border-gray-300 shadow-sm py-3 border focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
            <button type="submit"
              class="rounded-md bg-gray-200 px-4 text-sm font-medium text-gray-600 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-50">Apply</button>
          </div>
        </form>
    ...
    <section aria-labelledby="payment-heading"
      ...
      <div class="mx-auto max-w-lg lg:pt-16">
        <button type="button"
          class="flex w-full items-center justify-center rounded-md border border-transparent bg-black py-3 text-white hover:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-gray-900 focus:ring-offset-2">
        ...
        <form class="mt-6">
          ...
          <button type="submit"
            class="mt-6 w-full rounded-md border border-transparent bg-indigo-600 px-4 py-3 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Pay
            $141.92</button>
